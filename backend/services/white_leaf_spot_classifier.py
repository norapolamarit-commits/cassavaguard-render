"""Verified runtime loader for the experimental White Leaf Spot auxiliary head."""
...
METRICS_PATH = MODEL_DIR / "white_leaf_spot_metrics.json"
...
def _load() -> None:
    ...
    try:
        ...
        if metadata.get("classes") != ["other", "white_leaf_spot"]:
        ...
        if (
            ...
            raise ValueError("experimental artifact is not approved for production")
        ...
        probabilities = np.asarray(
            classifier.predict_proba(
                np.zeros((1, len(FEATURE_ORDER)), dtype=np.float64)
            )
        )
        if (
        ...
        _metrics = metadata
        print(
            f"[ai_engine] loaded experimental auxiliary '{metadata['model_id']}' "
            f"(test macro-F1={metadata['test']['macro_f1']:.3f})"
        )
    except FileNotFoundError:
        print("[ai_engine] White Leaf Spot auxiliary model is not trained yet")
    except Exception as exc:  # pragma: no cover - defensive fail-closed path
        _classifier = None
        _metrics = None
        print(f"[ai_engine] failed to load White Leaf Spot auxiliary model: {exc}")


def get_white_leaf_spot_classifier():
    with _access_lock:
        _load()
        return _classifier


def get_white_leaf_spot_metrics():
    with _access_lock:
        _load()
        return _metrics


def white_leaf_spot_predict_probability(
    features: dict,
    cnn_probabilities: dict,
) -> float:
    classifier = get_white_leaf_spot_classifier()
    if classifier is None:
        raise RuntimeError("White Leaf Spot auxiliary model is unavailable")
    vector = feature_vector(features) + [
        float(cnn_probabilities[name]) for name in ML_CLASS_ORDER
    ]
    return float(
        classifier.predict_proba(np.asarray([vector], dtype=np.float64))[0, 1]
    )
